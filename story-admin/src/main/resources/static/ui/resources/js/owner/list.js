 mini.parse();
    ...
    function edit(id){
        openWindow(Request.BASH_PATH + "admin/owner/save.html?id="+id, "编辑管理员", "70%", "80%", function (e) {
            grid.reload();
        });
    }
    function create() {
        openWindow(Request.BASH_PATH + "admin/owner/save.html", "新建管理员",  "70%", "80%", function (e) {
            grid.reload();
        });
    }

    function remove(id) {
        mini.confirm("确定删除此管理员", "确定？",
                function (action) {
                    if (action == "ok") {
                        grid.loading("删除中...");
                        Request['delete']("owner/" + id, {}, function (e) {
                            if (e.success) {
                                grid.reload();
                                showTips("删除成功!");
                            } else {
